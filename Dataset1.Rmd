...
An interaction plot can be produced to replicate @vanhedgerCricketChirpsCar2019's results, see figure 1.1. It should be noted that @vanhedgerCricketChirpsCar2019 perform two analysis's; (a) an ANOVA with all participants, and (b) an ANOVA with naïve participants only^[Relevant ANOVA descriptives can be found in supplementary table 1.].
...
lines(x = c(1, 2), y = c(cf0[1], cf0[1]+cf0[2]), lwd = 2, col = "skyblue")

## add CI to figures
crudeci_Inter <- posterior_interval(crude)[1, ] # extract posterior interval for
# soundscape = U
crudeci_Sound <- posterior_interval(crude)[2, ] # extract posterior interval for
# soundscape = N

# draw CI bands
arrows(x0 = 1, x1 = 1, 
       y0 = crudeci_Inter[1], y1 = crudeci_Inter[2], 
       length = 0.1, angle = 90, code = 3, lty = 1, col = "purple4", lwd = 2)
arrows(x0 = 2, x1 = 2, 
       y0 = crudeci_Sound[1], y1 = crudeci_Sound[2], 
       length = 0.1, angle = 90, code = 3, lty = 1, col = "purple4", lwd = 2)
...
lines(x = c(1, 2), y = c(cf1[1], cf1[1]+cf1[2]+cf1[3]), lwd = 2, col = "skyblue")

## add CI to figures
adjustedci_Inter <- posterior_interval(adjusted)[1, ] # extract posterior 
# interval for soundscape = U
adjustedci_Sound <- posterior_interval(adjusted)[2, ] # extract posterior 
# interval for soundscape = N

# draw CI bands
arrows(x0 = 1, x1 = 1, 
       y0 = adjustedci_Inter[1], y1 = adjustedci_Inter[2], 
       length = 0.1, angle = 90, code = 3, lty = 1, col = "purple4", lwd = 2)
arrows(x0 = 2, x1 = 2, 
       y0 = adjustedci_Sound[1], y1 = adjustedci_Sound[2], 
       length = 0.1, angle = 90, code = 3, lty = 1, col = "purple4", lwd = 2)
...
# New figure 1.2
# crude
jitt <- runif(nrow(d2), -.03, .03)
plot(as.numeric(d2$soundscape_type) + jitt, d2$CogTest, xlab="Soundscape Type",
ylab="Composite score", bty="l", pch=20, xaxt="n", yaxt="n")

sims <- as.matrix(crude)
n_sims <- nrow(sims)
sims_display <- sample(n_sims, 10)

CogTest_bar <- mean(d2$CogTest)

for (i in sims_display){
curve(cbind(1, x, CogTest_bar) %*% sims[i,1:3], lwd=0.5, col="gray", add=TRUE)
}
curve(cbind(1, x) %*% coef(crude), col="black", add=TRUE)
...
WC # Character count is 4309 and thus NOT< 4000 == BUT I referenced all packages
