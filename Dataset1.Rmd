...
## create CI function
add_ci <- function(m, c = 2, coef = 2, xpos = 1){
  point_est <- m$coefficients[c]  
  ci89 <- posterior_interval(m, level = 0.89)[coef, ]         # # 89 % CI
  
  # Draw CI 89 %
  arrows(x0 = xpos, x1 = xpos, y0 = ci89[1], y1 = ci89[2], 
         angle = 90, code = 3, length = 0.0)
    # Draw point estimate
  points(xpos, point_est, pch = 21, bg = "skyblue")
}
...
## add CI 
add_ci(crude, c = 1, coef = 1, xpos = 1)
# add_ci(crude, c = c(1+2), coef = c(1+2), xpos = 2)
...
## add ci
# c = coeficients in point estimate
# coef= post interval
add_ci(adjusted, c = 1, coef = c(2,3), xpos = 1)
# add_ci(adjusted, c = c(2, 3), coef = c(1, 2,3), xpos = 2)
# adjusted$coefficients[c(2+3)]
# posterior_interval(adjusted)[c(1, 2,3), ]
```
